...
import { DynamoProductService } from './dynamoProductService';
...
export class ProductService {
  ...
  static async getAllProducts(): Promise<Product[]> {
    try {
      console.log('🔍 ProductService.getAllProducts called (e-commerce site)');

      // Try DynamoDB first if online
      if (this.isOnline) {
        try {
          console.log('🌐 Attempting to fetch products from DynamoDB...');
          const products = await DynamoProductService.getAllProducts();
          // Cache the results
          this.saveProductsToStorage(products);
          console.log('✅ ProductService.getAllProducts returning', products.length, 'products from DynamoDB');
          return products;
        } catch (dynamoError) {
          console.warn('⚠️ DynamoDB unavailable, falling back to localStorage');
          console.log('💡 This is normal for e-commerce site if AWS auth not configured');
        }
      } else {
