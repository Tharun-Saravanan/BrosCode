...
export class CartService {
  ...
  // Get cart items for a specific user (async method with API)
  static async getUserCart(userId: string): Promise<CartItem[]> {
    try {
      // Try API first
      const apiItems = await ApiCartService.getUserCart(userId);
      if (apiItems.length > 0) {
        // Save to localStorage for offline access
        this.saveUserCartToStorage(userId, apiItems);
        return apiItems;
      }
      ...
      if (localItems.length > 0) {
        ...
        await ApiCartService.syncCart(userId, localItems);
      }
      return localItems;
    } catch (error) {
      console.error('Error getting user cart from API:', error);
  ...
  // Save cart items for a specific user (async method with API)
  static async saveUserCart(userId: string, items: CartItem[]): Promise<void> {
    try {
      ...
      await ApiCartService.syncCart(userId, items);
    } catch (error) {
      console.error('Error saving user cart to API:', error);
      // localStorage already saved, so cart is still persisted locally
    }
  }
